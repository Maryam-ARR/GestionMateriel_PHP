...
<div class="row justify-content-center">
    <form method="GET">
    <fieldset>
        <legend >Chercher Matériel par Désignation</legend>
    <div class="form-group">
    <label>Désignation</label><br>
    <input type="designation" name="designation" class="form-control" value="">
    </div>
...
$Designation=$_GET['designation'];
  if($Designation=='' ){
    $_SESSION['message']="veuillez entrer la désignation !!";
    $_SESSION['msg_type']="danger";
    header("location:chercherDesignation.php");
}
else{
 $mysqli=new mysqli('localhost','root','','gestion materiel','3308') or die(mysqli_error($mysqli));
 $result=$mysqli->query("SELECT * FROM `materiels scientifiques`Where Designation='$Designation'")or die($mysqli->error);
 
 if(!mysqli_num_rows($result)){
    $_SESSION['message']="La désignation n'exist pas !!";
    $_SESSION['msg_type']="danger";
    header("location:chercherDesignation.php");
} else{
  ?>
