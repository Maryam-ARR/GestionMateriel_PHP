...
<div class="row justify-content-center">
    <form method="GET">
    <fieldset>
        <legend >Chercher Matériel par DATE</legend>
    <div class="form-group">
    <label>Date d'Acquisition</label><br>
...
if(isset($_GET['Chercher'])):
 $Date=$_GET['date'];
  if($Date==''){
    $_SESSION['message']="veuillez entrer la date d'aquisition!!";
    $_SESSION['msg_type']="danger";
    header("location:chercherDate.php");
}
else{
     $mysqli=new mysqli('localhost','root','','gestion materiel','3308') or die(mysqli_error($mysqli));
     $result=$mysqli->query("SELECT * FROM `materiels scientifiques`Where date_aquisition='$Date'")or die($mysqli->error);
    
     if(!mysqli_num_rows($result)){
        $_SESSION['message']="La Date n'exist pas !!";
        $_SESSION['msg_type']="danger";
        header("location:chercherDate.php");
    } else{
  ?>
